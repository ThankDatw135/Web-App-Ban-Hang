...
export default function RegisterPage() {
  ...
  const CheckIcon = ({ checked }: { checked: boolean }) => (
    <span className={`transition-colors duration-200 ${checked ? 'text-green-400' : 'text-gray-500'}`}>
      ...
      <div className="relative z-10 flex min-h-screen flex-col">
        ...
        <header className="flex items-center justify-between px-6 py-6 md:px-12 w-full">
          ...
          <nav className="flex gap-4">
            ...
            <Link 
              href="/auth/login" 
              className="flex min-w-[100px] cursor-pointer items-center justify-center overflow-hidden rounded-lg bg-[#393328]/50 backdrop-blur-sm px-4 py-2 text-white text-sm font-bold leading-normal tracking-[0.015em] border border-transparent hover:border-muted-gold/50 transition-all duration-300"
            >
              <span>Log In</span>
            </Link>
          </nav>
        ...
        <main className="flex flex-1 items-center justify-center p-4 py-8">
          <div className="w-full max-w-[480px] flex flex-col">
            <div className="bg-[#221c10]/80 backdrop-blur-md border border-[#393328] rounded-xl shadow-2xl p-8 md:p-10 relative overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                  <label className="text-white text-sm font-medium leading-normal ml-1" htmlFor="fullName">
                    Full Name
                  </label>
                  <input 
                    id="fullName"
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-white text-sm font-medium leading-normal ml-1" htmlFor="password">
                    Password
                  </label>
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 pt-2 pl-1">
                    <div className={`flex items-center gap-2 text-xs transition-colors duration-200 ${passwordChecks.minLength ? 'text-green-400' : 'text-gray-500'}`}>
                      <CheckIcon checked={passwordChecks.minLength} />
                      <span>Min. 8 characters</span>
                    </div>
                    <div className={`flex items-center gap-2 text-xs transition-colors duration-200 ${passwordChecks.hasUpperLower ? 'text-green-400' : 'text-gray-500'}`}>
                      <CheckIcon checked={passwordChecks.hasUpperLower} />
                      <span>Uppercase & lowercase</span>
                    </div>
                    <div className={`flex items-center gap-2 text-xs transition-colors duration-200 ${passwordChecks.hasNumber ? 'text-green-400' : 'text-gray-500'}`}>
                      <CheckIcon checked={passwordChecks.hasNumber} />
                      <span>One number</span>
                    </div>
                    <div className={`flex items-center gap-2 text-xs transition-colors duration-200 ${passwordChecks.hasSpecial ? 'text-green-400' : 'text-gray-500'}`}>
                      <CheckIcon checked={passwordChecks.hasSpecial} />
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-white text-sm font-medium leading-normal ml-1" htmlFor="confirmPassword">
                    Confirm Password
                  </label>
                  <div className="relative flex w-full items-center">
                    <input 
              ...
              >
                ...
                <span className="text-white group-hover:text-muted-gold transition-colors">Sign Up with Google</span>
              </button>

              <div className="flex items-center justify-center gap-2 mt-6">
                <span className="text-gray-400 text-sm">Already have an account?</span>
                <Link href="/auth/login" className="text-white text-sm font-bold hover:text-muted-gold transition-colors">
                  Log In
                </Link>
              </div>
            </div>
